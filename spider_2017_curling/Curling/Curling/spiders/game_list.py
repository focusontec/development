...
class GameListSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # todo changed
        game_of_17 = response.xpath('(//tr[@class="odd"]|//tr[@class="even"])/td[7]')
        item = dict()
        item["season_year"] = "2017"
    ...
    def get_event_id(self,response):
        ...
        if event_id :
            ...
            get_eventCode_url = 'http://odf2.worldcurling.co/data/{}-config.html?_={}'.format(game_list['event_id'], int(
                round(time.time() * 1000)))
            yield scrapy.Request(url=get_eventCode_url, callback=self.get_final_result,
                                 meta={"game_list_dic": deepcopy(game_list)})
        else:
            game_list['event_id'] = ""
            game_list_item = dict()
            game_list_item['table_name'] = 'event_list_copy'
    ...
    def get_final_result(self,response):
        game_list = response.meta.get('game_list_dic')
        res = json.loads(response.text)
        print res,'++++++++++++++++++++++++++++++++'
        eventCode = res.get('eventCode')
        game_list['event_abbr'] = eventCode
        game_list_item = dict()
        game_list_item['table_name'] = 'event_list_copy'
        game_list_item['metric_pk'] = game_list.get('vc_md5')
        game_list_item['data_rows'] = [dict(game_list)]
